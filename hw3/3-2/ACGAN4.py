...
from ACGAN_Discriminator4 import MyDiscriminator
import os
...
class ACGAN():
    def __init__(self, width=64, height=64, channels=3, clip_value=0.01, latent_dim = 300, text_dim = 130, D_update_times = 1):
        ...
        self.optimizer_G = torch.optim.RMSprop(self.G.parameters(), lr = 0.0002)
        self.optimizer_D = torch.optim.RMSprop(self.D.parameters(), lr = 0.0001)
    ...
    def train_D(self, batch):
        ...
        score_loss = -real_score.mean() + fake_score.mean()
...
#             else:    
            ...
            if self.epoch % save_interval == 0:
                ...
                }, dir_name+"/W_ACGANcurrent")
                #self.plot_images(save2file=True, step=self.epoch)
                tags = np.zeros((25,130))
                tags[:,89] = 1
                self.plotGirls(tags, "black_hair_purple_eyes"+str(self.epoch)+".png")
                tags = np.zeros((25,130))
                tags[:,94] = 1
                self.plotGirls(tags, "pink_hair_black_eyes"+str(self.epoch)+".png")
                tags = np.zeros((25,130))
                tags[:,121] = 1
                self.plotGirls(tags, "red_hair_red_eyes"+str(self.epoch)+".png")
                tags = np.zeros((25,130))
                tags[:,100] = 1
                self.plotGirls(tags, "aqua_hair_green_eyes"+str(self.epoch)+".png")
                tags = np.zeros((25,130))
                tags[:,2] = 1
                self.plotGirls(tags, "blonde_hair_orange_eyes"+str(self.epoch)+".png")
    ...
    def plotGirls(self, tags, suffix):
        ...
        images = self.G(self.fixed_noise[:len(tags)], tags_tensor)
        #images is in -1 ~ 1
        rows = 5
        #turn -1 ~ 1 to 0 ~ 255
        image = (1 + images.cpu().detach().numpy())/2 * 255
